/*
 * Leisound V1 — GT911 Interrupt Mode Test (Software I2C bit-banging)
 *
 * Uses a bit-banged I2C driver (see gt911.c) instead of the ESP-IDF hardware
 * I2C master. This sidesteps the GPIO45 (strapping pin) pull-down issue that
 * breaks the hardware I2C driver on the V1.1 board.
 *
 * Data flow:
 *   GT911 INT pin (GPIO39) falling edge → GPIO ISR → semaphore
 *   → gt911_irq_task (FreeRTOS) → software I2C scan → user callback → log
 *
 * Pins (Leisound V1.1):
 *   EN_POWER  GPIO43   (LCD power rail)
 *   I2C SDA   GPIO38
 *   I2C SCL   GPIO45
 *   TP INT    GPIO39
 *   TP RST    GPIO40
...
void app_main(void)
{
    ESP_LOGI(TAG, "===== GT911 Interrupt Mode Test (Software I2C) =====");
     ...
     *    (software I2C pins are initialized inside gt911_init) */
    gt911_config_t cfg = {
        .rst_pin       = PIN_TP_RST,
        .int_pin       = PIN_TP_INT,
