        ...
        for f in self.files:
            if self.cf in (ColorFormat.RAW, ColorFormat.RAW_ALPHA):
                ...
                if self.resize:
                    ...
                    temp_file = f + ".resized.png"
                    with Image.open(f) as img_pil:
                        # Parse size format AxB
                        width, height = map(int, self.resize.split('x'))
                        resized_img = img_pil.resize((width, height), Image.LANCZOS)
                        resized_img.save(temp_file)
                    # Use resized image
                    img = LVGLImage().from_png(temp_file, self.cf, background=self.background, rgb565_dither=self.rgb565_dither, nema_gfx=self.nema_gfx)
...
def main():
    ...
    parser.add_argument(
        '--resize',
        help="resize image to specified size, format: AxB (e.g. 32x32, 480x800)",
        metavar='SIZE')
...
#python .\LVGLImage.py .\imgs\_hardcore_news_resized.png --name app_news_cover --resize 480x800
#python .\LVGLImage.py .\imgs\_hardcore_news_resized.png --name app_news_cover --resize 80x80
