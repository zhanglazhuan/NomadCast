...
static lv_color_t app_icon_color(const char *name)
{
    ...
    if (name && strcmp(name, "Weather") == 0)  return lv_color_hex(0x4CAF50);
...
static void create_grid_container(lv_obj_t *container, int cols,
                                  ...
                                  lv_coord_t *out_icon)
{
    ...
    if (out_icon)   *out_icon   = icon_size;
}
                                         ...
                                         lv_coord_t icon_size)
{
    ...
    lv_obj_set_style_radius(icon_cont, icon_size / 4, LV_PART_MAIN);
    ...
    if (app->icon) {
        ...
        /* Scale the 60px glyph down inside the smaller 3-per-row tile so it keeps
         * the same image:tile ratio as the 2-per-row (large) mode (60px in 80px).
         * The default image pivot is its center, so it stays centered. */
        const lv_image_dsc_t *dsc = (const lv_image_dsc_t *)app->icon;
        lv_coord_t glyph = icon_size * 3 / 4;
        uint32_t zoom = (uint32_t)glyph * 256 / dsc->header.w;
    ...
    /* g_cjk_font (HarmonyOS Sans 16px) has full CJK coverage; the 14px Source Han
     * subset lacks 播/客/放/器/设/置, which rendered the titles as empty boxes. */
...
void launcher_home_ui(void)
{
    ...
    /* 0 = medium (3 per row, default); 1 = large (2 per row) */
    int cols = (flash_get_i32("settings", "icon_size", 0) == 1) ? 2 : 3;
    lv_coord_t cell_w, cell_h, gap, icon_size;
    create_grid_container(home_page.container, cols, &cell_w, &cell_h, &gap, &icon_size);
    ...
    if (app_count == 0) {
        ...
        lv_obj_set_grid_cell(lbl, LV_GRID_ALIGN_CENTER, 0, cols,
    ...
    for (int i = 0; i < app_count; i++) {
        ...
        create_app_grid_button(home_page.container, app, col, row, cell_w, gap, icon_size);
        visible++;
    }
